...
void NIsoMuon::initializeAnalyzer(){
  ...
  else if( TriggerInput == "SingleMuon" ) {
    IsoMuTriggerName = {
...
void NIsoMuon::executeEventFromParameter(AnalyzerParameter param){
  ...
  vector<Muon> Vmuons = SelectMuons(AllMuons, param.Muon_Veto_ID, 10., 2.4);
  ...
  for(int it_sign = 0; it_sign < 2; it_sign++) {
    ...
    JSFillHist( str_sign+param.Name, "dR_MuJet"+str_sign+param.Name, jets[min_dR_jet].DeltaR( *dimuon[ dimuon.size() - 1 - min_dR_mu ] ), jets[min_dR_jet].DeltaR( *dimuon[min_dR_mu] ), weight, 4000, 0., 4., 4000, 0., 4.);
...
std::vector<Lepton*> NIsoMuon::NIsoMuonSelection(vector<Muon>& muons, vector<Muon>& Vmuons, int sgn) {
  ...
  for(unsigned i=0; i<muons.size(); i++) {
    ...
    for(unsigned j=1; j<Vmuons.size(); j++) {
